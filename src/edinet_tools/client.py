...
class EdinetClient:
    ...
    def get_recent_filings(
        ...
        for i in range(days_back):
            ...
            try:
                ...
                if doc_types:
                    daily_filings = [
                        doc for doc in daily_filings if doc.get("docTypeCode") in doc_types
                    ]
    ...
    def get_company_filings(
        ...
        company_filings = [doc for doc in recent_filings if doc.get("edinetCode") == edinet_code]
    ...
    def download_filing_raw(self, doc_id: str, raise_on_error: bool = True) -> bytes | None:
        ...
        try:
            ...
            if self._is_json_error_response(doc_response):
                ...
                logger.error(
                    f"API returned error for document {doc_id}: {status} - {error_message}"
                )
            ...
            if not self._is_zip_response(doc_response):
                ...
                raise ProcessingError(
                    f"Invalid response format for document {doc_id}",
                    doc_id,
