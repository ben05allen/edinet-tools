...
def parse_securities_report(
    ...
    operating_income = _coalesce(
        ...
        None
        if accounting_standard in ("IFRS", "US GAAP")
    ...
    nav_str = coerce_numeric_value(
        extract_value(csv_files, ELEMENT_MAP["net_assets_per_share"], context_patterns=patterns)
    )
    if not nav_str:
        nav_str = coerce_numeric_value(
            extract_value(
    ...
    eps_str = coerce_numeric_value(
        extract_value(csv_files, ELEMENT_MAP["earnings_per_share"], context_patterns=patterns)
    )
    if not eps_str:
        eps_str = coerce_numeric_value(
            extract_value(
    ...
    if not equity_str:
        equity_str = extract_value(
            csv_files, ELEMENT_MAP["equity_ratio_ifrs"], context_patterns=patterns
        )
    if not equity_str:
        equity_str = extract_value(
            csv_files, ELEMENT_MAP["equity_ratio_usgaap"], context_patterns=patterns
        )
    ...
    ifrs_eps_str = coerce_numeric_value(
        extract_value(
            csv_files,
            ELEMENT_MAP["earnings_per_share_ifrs"],
            context_patterns=["CurrentYearDuration"],
        )
    )
    ifrs_summary_basic_eps = Decimal(ifrs_eps_str) if ifrs_eps_str else None

    ifrs_roe_str = coerce_numeric_value(
        extract_value(
            csv_files,
            ELEMENT_MAP["roe_ifrs"],
            context_patterns=["CurrentYearDuration"],
        )
    )
    ifrs_summary_roe = Decimal(ifrs_roe_str) if ifrs_roe_str else None

    ifrs_bps_str = coerce_numeric_value(
        extract_value(
            csv_files,
            ELEMENT_MAP["bps_ifrs"],
    ...
    segments, segments_text_only, segments_extraction_incomplete = parse_segments_from_csv(
        csv_files
    )
