...
class EdinetXbrlCsvParser:
                ...
                if len(row) >= 9:  # Ensure we have all required columns (updated from 11 to 9)
                    try:
                        if total_rows <= 3:  # Show raw data for first few rows
                            logger.debug(
                                f"Raw row {row_num}: {[col[:50] for col in row[:3]]}"
                        ...
                        if extract_text_blocks and self._is_text_block(element_name):
                            ...
                            if metric:
                                ...
                                if self._is_relevant_metric(metric.element_name):
                                    ...
                                    total_rows <= 10 and metric.element_name
                                ):  # Show more examples for debugging
                                    logger.debug(
    ...
    def _parse_csv_row(self, row: list[str], total_rows: int = 0) -> FinancialMetric | None:
        ...
        try:
            ...
            if (
                total_rows <= 50
                and self._is_relevant_metric(element_name)
                and len([m for m in self.metrics if self._is_relevant_metric(m.element_name)]) == 0
            ):
                logger.debug("First relevant metric found:")
    ...
    def _format_text_blocks(self) -> dict[str, Any]:
        ...
        for block in self.text_blocks:
            ...
            if not block_key:
                ...
                block_key = (
                    block.element_name.split(":")[-1]
                    if ":" in block.element_name
                    else block.element_name
                )
    ...
    def _extract_key_metrics(self) -> dict[str, Any]:
        ...
        for metric_key, element_name in self.FINANCIAL_METRICS.items():
            ...
            for metric in self.metrics:
                if metric.element_name == element_name:
                    logger.debug(
                        f"Found metric {metric_key}: element={metric.element_name}, context={metric.context}, value={metric.value}"
                    )
            ...
            if current_value is not None or prior_value is not None:
                ...
                logger.debug(
                    f"Extracted metric {metric_key}: current={current_value}, prior={prior_value}"
                )
        ...
        logger.info(
            f"Extracted {len(result['financial_metrics'])} financial metrics from XBRL data"
        )
        return result
