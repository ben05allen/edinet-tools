...
class SemiAnnualReportProcessor(BaseDocumentProcessor):
    ...
    def process(self) -> StructuredDocumentData | None:
        ...
        if not structured_data["text_blocks"]:
            logger.info(
                f"Using generic text block extraction for {self.doc_id} (document may be investment trust or non-standard format)."
            )
...
class SecuritiesReportProcessor(BaseDocumentProcessor):
    ...
    def process(self) -> StructuredDocumentData | None:
        ...
        logger.debug(
            f"Finished processing Securities Report {self.doc_id}. Extracted {len(key_facts)} key facts, {len(structured_data['financial_tables'])} financial tables, and {len(structured_data['text_blocks'])} text blocks."
        )
        return structured_data if structured_data else None
    ...
    def _extract_financial_metrics(self) -> dict[str, Any]:
        ...
        for xbrl_id, metric_key in common_metrics.items():
            ...
            if current_value is None and prior_value is None:
                ...
                if current_value is not None or prior_value is not None:
                    financial_metrics[metric_key] = {"current": current_value, "prior": prior_value}
        ...
        for xbrl_id, table_title in table_elements.items():
            ...
            if table_content:
                financial_tables.append(
                    {"id": xbrl_id, "title": table_title, "content": table_content}
                )
    ...
    def _categorize_element(self, element_id: str) -> str:
        ...
        if any(
            keyword in element_id.lower()
            for keyword in ["business", "segment", "product", "service"]
        ):
            return "business_overview"
        elif any(
            keyword in element_id.lower() for keyword in ["risk", "uncertainty", "contingency"]
        ):
            return "risk_factors"
        elif any(
            keyword in element_id.lower()
            for keyword in ["management", "analysis", "md&a", "financial_position"]
        ):
            return "management_analysis"
        elif any(
            keyword in element_id.lower()
            for keyword in ["corporate", "governance", "director", "officer"]
        ):
            ...
            keyword in element_id.lower()
            for keyword in ["accounting", "policy", "standard", "method"]
        ):
...
class InternalControlReportProcessor(BaseDocumentProcessor):
    ...
    def process(self) -> StructuredDocumentData | None:
        ...
        structured_data[
            "financial_tables"
        ] = []  # Internal control reports don't have financial tables
        structured_data["text_blocks"] = self.get_all_text_blocks()

        logger.debug(
            f"Finished processing Internal Control Report {self.doc_id}. Extracted {len(key_facts)} key facts and {len(structured_data['text_blocks'])} text blocks."
        )
        return structured_data if structured_data else None
