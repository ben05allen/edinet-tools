...
def test_jgaap_control_unaffected():
    ...
    r = parse_securities_report(
        csv_files=_load("jgaap_control_revenue"), doc_id="TEST", doc_type_code="120"
    )
...
def test_usgaap_income_statement_mapped_sony():
    ...
    r = parse_securities_report(
        csv_files=_load("sony_fy20_usgaap_revenue"), doc_id="TEST", doc_type_code="120"
    )
    ...
    assert (
        r.operating_income == 845_459_000_000
...
def test_strict_consolidated_nulls_are_honest_mhi():
    ...
    """
    r = parse_securities_report(
        csv_files=_load("mhi_ifrs_blast_radius"), doc_id="TEST", doc_type_code="120"
    )
    ...
    assert any(
        getattr(f, "value", "") == "1947178000000"
        and "NonConsolidatedMember" in (getattr(f, "context_id", "") or "")
        for f in (r.raw_facts or [])
    ), "parent value must remain in the fact-bag"
