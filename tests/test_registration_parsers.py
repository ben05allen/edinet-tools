...
class TestSecuritiesRegistrationCoverPageFields:
    ...
    def test_extracts_contact_person(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["contact_person"], "FilingDateInstant", "運用部　佐藤花子"
            )
        ]
    ...
    def test_extracts_telephone_number(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["telephone_number"], "FilingDateInstant", "03-5555-1234"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_place_of_filing(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["place_of_filing"], "FilingDateInstant", "関東財務局長"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_amount_to_register(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["amount_to_register"], "FilingDateInstant", "1口以上1口単位"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_fund_name_for_registration(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_name_for_registration"],
                "FilingDateInstant",
                "テスト日本株式ファンド",
            )
        ]
...
class TestSecuritiesRegistrationFundIdentityFields:
    ...
    def test_extracts_fund_scheme(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_scheme"], "FilingDateInstant", "単位型・追加型：追加型"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_fund_history(self):
        rows = [
            _make_reg_csv_row(REG_ELEMENT_MAP["fund_history"], "FilingDateInstant", "2000年1月設定")
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_fund_identity_in_text_blocks_dict(self):
        ...
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_purpose"], "FilingDateInstant", "ファンドの目的"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_scheme"], "FilingDateInstant", "ファンドの仕組み"
            ),
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
...
class TestSecuritiesRegistrationInvestmentPolicyFields:
    ...
    def test_extracts_investment_policy(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["investment_policy"],
                "FilingDateInstant",
                "主として国内株式に投資します。",
            )
        ]
    ...
    def test_extracts_investment_risks(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["investment_risks"],
                "FilingDateInstant",
                "株価変動リスクがあります。",
            )
        ]
    ...
    def test_extracts_eligible_investments(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["eligible_investments"], "FilingDateInstant", "国内上場株式"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_investment_restrictions(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["investment_restrictions"], "FilingDateInstant", "同一銘柄10%以内"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
...
class TestSecuritiesRegistrationOperationsFields:
    ...
    def test_extracts_management_fees(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["management_fees"], "FilingDateInstant", "年率1.0%（税込）"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_application_fee(self):
        rows = [
            _make_reg_csv_row(REG_ELEMENT_MAP["application_fee"], "FilingDateInstant", "3.3%以内")
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_application_period(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["application_period"], "FilingDateInstant", "毎営業日"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_application_unit(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["application_unit"], "FilingDateInstant", "1口以上1口単位"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_redemption_procedures(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["redemption_procedures"], "FilingDateInstant", "翌営業日解約"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_profit_distribution_policy(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["profit_distribution_policy"], "FilingDateInstant", "毎月分配"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_taxation(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["taxation"], "FilingDateInstant", "普通分配金は課税対象"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
...
class TestSecuritiesRegistrationFinancialFields:
    ...
    def test_extracts_financial_info(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["financial_info"], "FilingDateInstant", "純資産総額：100億円"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_balance_sheet(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["balance_sheet"], "FilingDateInstant", "資産合計：100億円"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_income_statement(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["income_statement"], "FilingDateInstant", "当期純利益：5億円"
            )
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
    ...
    def test_extracts_changes_in_net_assets(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["changes_in_net_assets"],
                "FilingDateInstant",
                "期末純資産総額：100億円",
            )
        ]
    ...
    def test_extracts_net_assets_calculation(self):
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["net_assets_calculation"],
                "FilingDateInstant",
                "1口当たり純資産額：10,000円",
            )
        ]
    ...
    def test_financial_blocks_in_text_blocks_dict(self):
        ...
        rows = [
            _make_reg_csv_row(
                REG_ELEMENT_MAP["balance_sheet"], "FilingDateInstant", "資産合計：50億円"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["income_statement"], "FilingDateInstant", "利益：1億円"
            ),
        ]
        report = parse_securities_registration(_make_reg_doc(rows=rows))
...
class TestSecuritiesRegistrationFullIntegration:
    ...
    def test_full_filing_extraction(self):
        rows = [
            ...
            _make_reg_csv_row(
                REG_ELEMENT_MAP["issuer_name"],
                "FilingDateInstant",
                "大和アセットマネジメント株式会社",
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["contact_person"], "FilingDateInstant", "運用部　田中"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["telephone_number"], "FilingDateInstant", "03-5555-9999"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["place_of_filing"], "FilingDateInstant", "関東財務局長"
            ),
            ...
            # Fund identity
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_name"], "FilingDateInstant", "ダイワ日本株式オープン"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["fund_purpose"], "FilingDateInstant", "国内株式に投資するファンド"
            ),
            # Investment policy
            _make_reg_csv_row(
                REG_ELEMENT_MAP["investment_policy"], "FilingDateInstant", "国内株式への分散投資"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["investment_risks"], "FilingDateInstant", "価格変動リスク"
            ),
            # Operations
            _make_reg_csv_row(
                REG_ELEMENT_MAP["management_fees"], "FilingDateInstant", "年率0.99%（税込）"
            ),
            _make_reg_csv_row(
                REG_ELEMENT_MAP["redemption_procedures"], "FilingDateInstant", "翌営業日解約可能"
            ),
            # Financial
            _make_reg_csv_row(
                REG_ELEMENT_MAP["balance_sheet"], "FilingDateInstant", "純資産：200億円"
            ),
        ]
...
class TestMinimalRegistrationParsers:
    ...
    def test_securities_notification_repr_amendment(self):
        report = SecuritiesNotificationReport(doc_id="X", doc_type_code="020", is_amendment=True)
        assert "AMENDED" in repr(report)
