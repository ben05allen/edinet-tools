...
class TestRealAPIContracts:
    ...
    def setup_method(self):
        ...
        if not self.api_key:
            pytest.skip(
                "❌ EDINET_API_KEY NOT FOUND - integration tests SKIPPED (set API key in .env file)"
            )

        # Check if API key appears to be valid format
        if len(self.api_key.strip()) < 10:  # Basic length check
            pytest.skip(
                f"❌ API KEY TOO SHORT: {len(self.api_key)} chars (expected >10) - integration tests SKIPPED"
            )
    ...
    def test_api_error_handling_with_invalid_key(self):
        ...
        assert "statusCode" in result or "message" in result, (
            "Error response should have statusCode or message"
        )
        ...
        assert (
            "401" in response_str
            or "unauthorized" in response_str
            or "access denied" in response_str
            or "subscription key" in response_str
        )
