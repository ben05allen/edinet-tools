...
auditor_opinion\tUnqualified"""
    ...
    def test_zip_end_to_end_doc_180_real_processing(self):
        ...
        rows = [
            ...
            (
                ...
                "取締役会決議",
                "CurrentYearInstant",
                "2025年8月1日開催の取締役会において、A社の全株式取得を決議。",
            ),
            (
            ...
            (
                ...
                "業績への影響",
                "CurrentYearInstant",
                "当連結会計年度の業績に与える影響は軽微である。",
            ),
            (
                "jpcrp_cor:SubmissionReasonTextBlock",
                "提出理由",
                "CurrentYearInstant",
                "子会社取得のため臨時報告書を提出する。",
            ),
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            zf.writestr("jpcrp-esr-001_E99999-000.csv", csv_content.encode("utf-16"))
            # Auditor file that must be filtered:
            zf.writestr(
                "jpaud_audit_E99999.csv",
        ...
        assert result is not None, "process_zip_file returned None — real end-to-end pipeline broke"
    ...
    def test_zip_end_to_end_doc_220_treasury_real_processing(self):
        ...
        rows = [
            ...
            (
                "jpcrp_cor:TreasuryStockAcquisitionTextBlock",
                "自己株式取得",
                "CurrentYearInstant",
                "当社は本日、自己株式の取得を決議いたしました。",
            ),
        ]
        csv_content = "\n".join("\t".join(r) for r in rows)
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
