...
class TestSecuritiesReport:
    ...
    def test_element_map_has_ifrs_summary_entries(self):
        ...
        for key in [
            "net_sales_ifrs_summary",
            "net_income_ifrs_summary",
            "total_assets_ifrs_summary",
            "net_assets_ifrs_summary",
            "operating_income_ifrs_summary",
            "earnings_per_share_ifrs",
            "equity_ratio_ifrs",
            "roe_ifrs",
        ]:
            assert key in ELEMENT_MAP, f"Missing ELEMENT_MAP key: {key}"
...
class TestExtraordinaryReport:
    ...
    def test_dataclass_has_amendment_and_contact_fields(self):
        ...
        for key in [
            ...
            "place_of_filing_fund",
            "place_of_filing_corp",
            "contact_person_fund",
            "contact_person_corp",
        ]:
            assert key in ELEMENT_MAP, f"Missing ELEMENT_MAP key: {key}"
...
class TestExtractionUtilities:
        ...
        csv_files = [
            {
                ...
                "data": [
                    ...
                    {
                        "要素ID": "jppfs_cor:Assets",
                        "コンテキストID": "CurrentYearInstant_ConsolidatedMember",
                        "値": "1000000",
                    },
                ],
            }
    ...
    def test_extract_value_exact_context_matching(self):
        ...
        assert (
            extract_value(csv_files, "jppfs_cor:NetSales", context_patterns=["CurrentYearDuration"])
            is None
        )
        ...
        assert (
            extract_value(
                csv_files,
                "jppfs_cor:NetSales",
                context_patterns=["CurrentYearDuration_NonConsolidatedMember"],
            )
            == "174000000000"
        )
    ...
    def test_extract_financial_with_ifrs_fallback(self):
        ...
        result = extract_financial(
            csv_files, "jppfs_cor:Assets", "CurrentYearInstant", True, ifrs_map
        )
        ...
        csv_files = [
            {
                ...
                "data": [
                    {
                        "要素ID": "jpcrp_cor:BusinessDescriptionTextBlock",
                        "コンテキストID": "ctx1",
                        "値": "Business text",
                    },
                    {"要素ID": "jppfs_cor:Assets", "コンテキストID": "ctx1", "値": "1000000"},
                    {
                        "要素ID": "custom:SomeElement",
                        "コンテキストID": "ctx1",
                        "値": "custom value",
                    },
                ],
