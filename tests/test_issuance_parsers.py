...
class TestParseIssuanceSupplementary:
    ...
    def test_extracts_company_name(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["company_name"], "FilingDateInstant", "テスト株式会社"
            ),
        ]
    ...
    def test_extracts_remaining_balance(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["remaining_balance"], "FilingDateInstant", "500億円"
            ),
        ]
        report = parse_issuance_supplementary(_make_supp_doc(rows=rows))
    ...
    def test_extracts_security_types(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["security_types"], "FilingDateInstant", "普通社債券"
            ),
        ]
        report = parse_issuance_supplementary(_make_supp_doc(rows=rows))
    ...
    def test_extracts_parent_shelf_reg_number(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["parent_shelf_reg_number"], "FilingDateInstant", "第1号"
            ),
        ]
        report = parse_issuance_supplementary(_make_supp_doc(rows=rows))
    ...
    def test_company_name_falls_back_to_filer_name(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["filer_name"], "FilingDateInstant", "フォールバック会社"
            ),
        ]
        report = parse_issuance_supplementary(_make_supp_doc(rows=rows))
    ...
    def test_multiple_fields_extracted_together(self):
        rows = [
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["company_name"], "FilingDateInstant", "ABCホールディングス株式会社"
            ),
            _make_supp_csv_row(SUPP_ELEMENT_MAP["supplement_number"], "FilingDateInstant", "第2号"),
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["remaining_balance"], "FilingDateInstant", "300億円"
            ),
            _make_supp_csv_row(SUPP_ELEMENT_MAP["filing_date"], "FilingDateInstant", "2025-03-01"),
            _make_supp_csv_row(
                SUPP_ELEMENT_MAP["filer_edinet_code"], "FilingDateInstant", "E05678"
            ),
        ]
        report = parse_issuance_supplementary(_make_supp_doc(rows=rows))
