...
def _make_doc(
    doc_id: str = "S100TEST", doc_type: str = "235", rows: list[str] | None = None
) -> Mock:
    doc = Mock()
    doc.doc_id = doc_id
...
class TestFieldExtraction:
    ...
    def test_extracts_representative(self):
        rows = [
            _make_csv_row(
                ELEMENT_MAP["representative"], "FilingDateInstant", "代表取締役社長 山田 太郎"
            ),
        ]
        report = parse_internal_control(_make_doc(rows=rows))
    ...
    def test_all_cover_page_fields_extracted_together(self):
        rows = [
            ...
            _make_csv_row(
                ELEMENT_MAP["representative"], "FilingDateInstant", "代表取締役 佐藤 一郎"
            ),
