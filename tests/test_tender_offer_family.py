...
class TestTenderOfferWithdrawalReport:
    ...
    def test_repr_truncates_long_name(self):
        report = TenderOfferWithdrawalReport(doc_id="X", doc_type_code="260", filer_name="A" * 50)
...
class TestTenderOfferResultReport:
    ...
    def test_amendment_flag(self):
        report = TenderOfferResultReport(doc_id="TEST", doc_type_code="280", is_amendment=True)
    ...
    def test_repr(self):
        report = TenderOfferResultReport(
            doc_id="X", doc_type_code="270", acquirer_name="買付者", target_name="対象会社"
        )
        r = repr(report)
...
class TestOpinionReport:
    ...
    def test_repr(self):
        report = OpinionReport(doc_id="X", doc_type_code="290", filer_name="テスト会社")
...
class TestQuestionResponseReport:
    ...
    def test_amendment(self):
        report = QuestionResponseReport(doc_id="X", doc_type_code="320", is_amendment=True)
        assert report.is_amendment is True
        assert "AMENDED" in repr(report)

    def test_repr(self):
        report = QuestionResponseReport(doc_id="X", doc_type_code="310", filer_name="テスト会社")
...
class TestExemptionApplicationReport:
    ...
    def test_amendment(self):
        report = ExemptionApplicationReport(doc_id="X", doc_type_code="340", is_amendment=True)
        assert report.is_amendment is True
        assert "AMENDED" in repr(report)
...
class TestElementMaps:
    ...
    def test_values_are_namespaced(self, element_map, label):
        for key, elem_id in element_map.items():
            assert ":" in elem_id, f"{label}: element '{key}' missing namespace prefix: {elem_id}"
...
def _make_family_doc(
    doc_id: str = "S100TEST", doc_type: str = "270", rows: list[str] | None = None
):
    doc = Mock()
    doc.doc_id = doc_id
...
class TestParseTenderOfferResultReport:
    ...
    def test_result_text_block_captured(self):
        rows = [
            _make_csv_row(
                REPORT_MAP["result_text"], "FilingDateInstant", "公開買付けは成立しました。"
            ),
        ]
        report = parse_tender_offer_report(_make_family_doc(rows=rows))
...
class TestParseOpinionReport:
    ...
    def test_extracts_target_company_name(self):
        rows = [
            _make_csv_row(
                OPINION_MAP["target_company_name"], "FilingDateInstant", "株式会社ローソン"
            ),
        ]
    ...
    def test_multiple_fields_extracted_together(self):
        rows = [
            _make_csv_row(
                OPINION_MAP["target_company_name"], "FilingDateInstant", "テスト対象会社"
            ),
